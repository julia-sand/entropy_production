...
niters = 10 #iterations in sinkhorn algorithm
...
#do sinkhorn iterations
w2_dist,G0 = sinkhorn_pointcloud.sinkhorn_loss(torch.from_numpy(xs).reshape((n, 1)) , torch.from_numpy(xt).reshape((n, 1)) , 1, n, niters)
print("sinkhorn done")
#find the maximum and replace with ones and return it back to numpy
G0_out = np.zeros_like(G0.numpy())
G0_out[np.arange(len(G0.numpy())), G0.numpy().argmax(1)] = 1
